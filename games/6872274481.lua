		...
		if remoteName == remotes.AttackEntity then
			return {
				...
				SendToServer = function(_, attackTable, ...)
					...
					if suc and plr then
						if not (select(2, whitelist:get(ent.Player)) or select(2, koolwl:get(ent.Player.UserId))) then return end
					end
	...
	bedwars.BlockController.isBlockBreakable = function(self, breakTable, plr)
		...
		if obj and obj.Name == 'bed' then
			for _, plr in playersService:GetPlayers() do
				if obj:GetAttribute('Team'..(plr:GetAttribute('Team') or 0)..'NoBreak') and not (select(2, whitelist:get(ent.Player)) or select(2, koolwl:get(ent.Player.UserId))) then
					return false
				end
			end
		end
